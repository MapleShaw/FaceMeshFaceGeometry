...
class FaceMeshFaceGeometry extends BufferGeometry {
  constructor(options = {}) {
    ...
    this.trackPoint = new Vector3(); // 独立追踪点
  }
  ...
  update(face, cameraFlipped) {
    ...
    for (const p of face.scaledMesh) {
      let point = []
      if (p.x) { // mediapipe 的数据是 { x: , y: , z:  } 格式，tf 的数据是 [ x, y, x ] 格式
        point = [p.x, p.y, p.z]
      } else {
        point = p
      }
      this.positions[ptr] = cameraFlipped
        ? point[0] + 0.5 * this.w
        : point[0] - 0.5 * this.w
      this.positions[ptr + 1] = this.h - point[1] - 0.5 * this.h
      this.positions[ptr + 2] = -point[2]
      ptr += 3
    }
    if (this.useVideoTexture) {
      ...
      if (this.normalizeCoords) {
        ...
        for (const p of face.scaledMesh) {
          ...
          this.positions[ptr] = scale * (point[0] / this.w + 0.5)
          this.positions[ptr + 1] = scale * (-point[1] / this.h + 0.5) * ar
          this.positions[ptr + 2] = scale * (-point[2] / 500)
          ptr += 3
        }
      }
    } else {
  ...
  track(id0, id1, id2) {
    ...
    console.log('======this.p0, this.p1, this.p2======', this.p0, this.p1, this.p2)
    ...
    return { position: center, normal, rotation: matrix, scale: center.distanceTo(this.p0) }
  }
}
